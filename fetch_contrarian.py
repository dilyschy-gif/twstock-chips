...
def calc_institutional_streaks(gc):
    ...
    for row in rows:
        try:
            ...
            trust_val = row[col_map["trust"]] if "trust" in col_map else "0"
            foreign_val = row[col_map["foreign"]] if "foreign" in col_map else "0"
            total_val = row[col_map["total"]] if "total" in col_map else "0"
    ...
    for row in rows:
        try:
            ...
            results[code] = {
                ...
                "market": str(safe_get("market")).strip(),
    ...
    if candidates:
        ...
        for c in candidates[:50]:
            data_rows.append([
                ...
                c["volume_lots"], c["rel_strength"],
                c["trust_streak"], c["foreign_streak"],
                c["n_target"], c["signal"], c["bandwidth"],
                c["institutional_score"], c["ntheory_score"],
