...
v4 改動（2026-07-06，經瀏覽器實測驗證）：
  A. TWSE：改用 Session 先訪問 t86 網頁取 cookie 再打 API；
     被 WAF 回 307 打發時重建 session 重試（最多 3 次、間隔 10 秒）。
  B. TPEX：舊版 3itrade_hedge_result.php 已退役（永遠回空 aaData），
     改打新版端點 POST /www/zh-tw/insti/dailyTrade（必須 POST，GET 回空白），
     欄位對照 [10]外資合計 [13]投信 [22]自營合計 [23]三法人，
     已用 2026-07-03 全部 827 檔上櫃股驗算加總，0 檔不合。
v3 改動：
