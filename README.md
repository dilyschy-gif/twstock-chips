...
### 當沖選股雷達（day-trading.html）

`scripts/day_trading.py` 盤後抓 TWSE MI_INDEX（上市）與 TPEX dailyQuotes（上櫃）
全市場收盤行情，逐日落地 `public/data/day-trading/daily/YYYYMMDD.json`（滾動保留
50 個快照），再依三大條件掃出**隔日**當沖觀察名單寫入 `day-trading.json`：

1. **成交量大**：日成交量 ≥ 1,000 張，或日成交金額 ≥ 5,000 萬元
2. **波動度夠**：近 30 個交易日平均每日振幅 ≥ 3%（振幅 = (高-低)/前收；至少 20 天樣本）
3. **熱門題材**：純數據族群強度 —— 依產業族群當日加權漲幅(45%)、量比中位(30%)、
   成交占比(25%) 百分位排名，前 5 名且加權漲幅為正 = 熱門族群，成員即符合；
   或個股量比 ≥ 2 且成交金額 ≥ 1 億（焦點資金股）也算。

只收 4 碼純數字普通股（ETF/權證/DR/特別股剔除）。前收優先用「收盤−漲跌價差」推回，
缺漏退用前一快照收盤，都沒有就不算該日振幅。兩市場各設最低檔數門檻（上市 700／
上櫃 500），任一市場沒到就整天不寫快照——半套資料會毀掉族群排名。產業別來自
mopsfin t187ap03_L CSV＋TPEX OpenAPI（openapi.twse.com.tw 擋資料中心 IP，勿改回），
抓不全沿用 `industry.json` 舊檔。TWSE TWTB4U 當沖率為加分顯示，抓不到不影響名單。
兩條件觀察股也會進 JSON（前端預設只顯示三條件全中），名單上限 300 檔。

排程台北 15:30 與 17:45 各一次（第二次補抓較晚產出的行情，已抓過的日期自動跳過）。
**首次建置**：手動跑 workflow「每日當沖選股雷達」填 backfill=60，湊滿 30+ 個交易日。
規則測試：`python -m unittest test_day_trading -v`（不連網，合成快照）。
